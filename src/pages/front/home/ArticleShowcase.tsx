...
const ArticleShowcase: React.FC = () => {
  ...
  const [leftHeight, setLeftHeight] = useState<number | string>('auto');
  const itemRefs = useRef<(HTMLDivElement | null)[]>([]);
  const showcaseRightRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    const updateHeight = () => {
      // 倒数第三个元素的索引
      const targetIndex = articles.length - 3;
      if (targetIndex >= 0 && itemRefs.current[targetIndex]) {
        const targetItem = itemRefs.current[targetIndex];
        // 获取该元素相对于 showcase-right 容器的偏移位置
         // 由于 article-list-item 是 relative 且在 list-items 内部
         // 我们计算它的 offsetTop + offsetHeight，使左侧底部对齐到倒数第三个元素的底部
         const offsetTop = targetItem.offsetTop;
         const offsetHeight = targetItem.offsetHeight;
         // 增加限制：左边容器的高度等于倒数第三个元素的底部位置
         // 这样 sticky 元素在到达这个高度时就会停止跟随
         setLeftHeight(offsetTop + offsetHeight);
       }
     };
  ...
  return (
    <div className="article-showcase-container">
      <div className="showcase-content">
        ...
        <div className="showcase-left" style={{ height: leftHeight }}>
        ...
        <div className="showcase-right" ref={showcaseRightRef}>
